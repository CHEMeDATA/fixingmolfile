...
let svg_data;
let im;
let imageOfStructuresvg;
let contentSdf;
let page;
let fileNameSdfFile;
let pageFile;
for (let index = 0; index < 1000; index++) {
  im = "input/images/struct" + index + ".png";// only this numbering starts at 0 - the others at 1
  if (existsSync(im)) {
    image_input = readFileSync(im);
    imageOfStructuresvg = "input/separated_svg/str" + (index + 1) + ".svg";
    ...
    results.push({
      inPngImage: image_input,
      outSvgImage: svg_data,
      sdf: contentSdf,
      pageNumber: page
    });
